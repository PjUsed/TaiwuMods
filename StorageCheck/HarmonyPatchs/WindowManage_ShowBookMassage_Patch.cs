	...
	public static class WindowManage_ShowBookMassage_Patch
	{
		public static string getBookInfo(WindowManage __instance, int itemId)
		{
			...
			else
			{
				...
				int[] array = ((num == 17)
					? ((!DateFile.instance.gongFaBookPages.ContainsKey(key)) ? new int[10] : DateFile.instance.gongFaBookPages[key])
					: ((!DateFile.instance.skillBookPages.ContainsKey(key)) ? new int[10] : DateFile.instance.skillBookPages[key]));
				...
				foreach (int key2 in array5)
				{
					...
					{
						if (!(DateFile.instance.GetItemDate(key3, 999, true, -1) == num2.ToString()) && (!StorageCheck.Settings.ShowBookPage.Value || int.Parse(DateFile.instance.GetItemDate(key3, 32, true, -1)) != num3))
						{
							continue;
						}
						...
						for (int j = 0; j < 10; j++)
						{
							...
							array2[j]++;
							if (StorageCheck.Settings.ShowBookPage.Value)
							{
				...
				if (bookPage.Length != 0)
				{
					...
					for (int k = 0; k < bookPage.Length; k++)
					{
						...
						text += string.Format("{0}{1}{2}{3}{4}",
							DateFile.instance.massageDate[10][2], DateFile.instance.massageDate[8][2].Split('|')[k],
							(bookPage[k] != 1) ? DateFile.instance.SetColoer(20010, DateFile.instance.massageDate[7010][4].Split('|')[0], false) : DateFile.instance.SetColoer(20004, DateFile.instance.massageDate[7010][4].Split('|')[1], false),
							(array[k] != 1 && array[k] > -100) ? (DateFile.instance.SetColoer(20002, $"  ({DateFile.instance.massageDate[7009][4].Split('|')[2]})", false) + DateFile.instance.SetColoer(10001, "  " + str2, false)) : (DateFile.instance.SetColoer(20005, $"  ({DateFile.instance.massageDate[7009][4].Split('|')[3]})", false) + DateFile.instance.SetColoer(10001, "  100%", false)),
							(array2[k] >= 1) ? (DateFile.instance.SetColoer(20004, $"  ○已有{array2[k]}页", false) + (array3[k] ? (DateFile.instance.SetColoer(20004, "真传", false) + (array4[k] ? DateFile.instance.SetColoer(20010, "/手抄", false) : "")) : (array4[k] ? DateFile.instance.SetColoer(20010, "手抄", false) : "")) + "\n") : DateFile.instance.SetColoer(20010, $"  ×无此页\n", false));
					}
					text += "\n";
				}
				if (num == 17)
